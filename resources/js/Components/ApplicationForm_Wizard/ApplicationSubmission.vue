...
const questions = {
    "The account holder as per credit information furnished, hereby gives Kovacs Student Village permission to do a credit worthiness evaluation": {
        type: "checkbox",
        agreement: true,
        name: "confirm_kovacs_credit_eva",
    },
    "I confirm that all the information furnished is complete and accurate": {
        type: "checkbox",
        agreement: true,
        name: "confirm_info_correct",
    },
}
        ...
        <div class="w-[100%]" v-for="(question, index) in Object.keys(questions)" :key="question">
            <div v-if="checkVisibility(questions[question])"
                ...
                <label :for="questions[question].name"
                    :class="`flex-1 ${questions[question].type === 'checkbox' && 'pl-8 md:pl-0'}`">
                    {{ question.split?.("-")?.[0] }}
                </label>
                ...
                <div v-else-if="(questions[question].type === 'checkbox')" class="absolute md:static">
